...
extension NSRect {
    var topLeft: CGPoint {
        set {
            ...
            // Pick the most likely target screen by testing which screen would
            // contain the window if we interpret newValue as "top-left within that screen".
            let targetScreen: NSScreen? = {
                let screens = NSScreen.screens
                if screens.isEmpty { return nil }

                var best: (screen: NSScreen, score: CGFloat)?
                for s in screens {
                    let sf = s.frame
                    // Convert top-left (y down) into Cocoa origin.y (y up) relative to this screen.
                    let candidateOrigin = CGPoint(
                        x: newValue.x,
                        y: sf.maxY - newValue.y - size.height
                    )
                    let candidateRect = CGRect(origin: candidateOrigin, size: size)
                    let score = area(candidateRect.intersection(sf))
                    if best == nil || score > best!.score {
                        best = (s, score)
                    }
                }
                return best?.screen ?? NSScreen.main ?? screens.first
            }()

            let screenFrameRect = targetScreen?.frame
                ?? NSScreen.main?.frame
                ?? NSScreen.screens.first?.frame
                ?? CGRect(x: 0, y: 0, width: 1920, height: 1440)
            origin.x = newValue.x
            origin.y = screenFrameRect.maxY - newValue.y - size.height
        }
        get {
            func area(_ r: CGRect) -> CGFloat {
                if r.isNull || r.isEmpty { return 0 }
            ...
            // Choose the screen this rect is mostly on (handles spanning screens).
            let targetScreen: NSScreen? = {
                let screens = NSScreen.screens
                if screens.isEmpty { return nil }
                ...
                var best: (screen: NSScreen, score: CGFloat)?
                for s in screens {
                    let score = area(self.intersection(s.frame))
                    if best == nil || score > best!.score {
                        best = (s, score)
                    }
                }
            ...
            let screenFrameRect = targetScreen?.frame
                ...
                ?? NSScreen.screens.first?.frame
                ?? CGRect(x: 0, y: 0, width: 1920, height: 1440)
            return CGPoint(x: origin.x, y: screenFrameRect.maxY - origin.y - size.height)
        }
    }
}
...
public class MultiWindowManager: NSObject, NSWindowDelegate {
    ...
    public static func createWindow(args: [String]) -> Int64 {
        if let RegisterGeneratedPlugins = MultiWindowManagerPlugin.RegisterGeneratedPlugins {
            ...
            // Keep secondary windows visible when the app deactivates.
            window.hidesOnDeactivate = false

            // Always create new windows on the main screen to avoid a Flutter/macOS
            // resize/metrics glitch that can occur when a secondary window is first
            // shown on a non-main display and then moved across displays.
    ...
    public func windowDidBecomeKey(_ notification: Notification) {
        if mainWindow is MultiWindowManagerFlutterWindow {
            emitEvent("focus")
        }
    }
