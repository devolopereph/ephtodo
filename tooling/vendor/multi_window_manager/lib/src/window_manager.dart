...
/// Event emitted by the native layer (both to the hiding window via _EmitEvent
/// and globally via _EmitGlobalEvent) when [isEnabledReuse] is set and the
/// window receives WM_CLOSE from any source (X button, Alt+F4, task manager
/// graceful close, etc.).
///
...
const _kWindowEventConfirmClose = 'confirm-close';
...
class MultiWindowManager {
  ...
  Future<dynamic> _methodCallHandler(MethodCall call) async {
    ...
    if (windowId != null) {
      ...
      if (eventName == _kWindowEventConfirmClose) {
        await _confirmClose();
        return;
      }
  ...
  /// Calls when [_kWindowEventConfirmClose]. Doing all async methods before close
  /// -> setConfirmClose(true) -> close()
  Future<void> _confirmClose() async {
    if (_isReusable) {
      await Future.wait(
        _reuseListeners.map((e) => e.onConfirmReuseCloseWindow()),
      );
    }
    if (_ipc != null) {
      await _ipc?.close();
  ...
  /// Get all alive engine window manager ids.(also reuse-hidden)
  static Future<List<int>> getAllAliveEngineIds() async {
    return (await _staticChannel
  ...
  /// Always `true` on linux cause GtkWindow can't corrects close secondary windows with flutter engine
  ...
  static Future<void> ensureInitializedSecondary(int windowId,
    ...
    if (_current != null) return;
    final isEnabledReuseLocal = Platform.isLinux || isEnabledReuse;
    final Map<String, dynamic> arguments = {
      'windowId': windowId,
      'isEnabledReuse': isEnabledReuseLocal,
    };
    // ignore: unused_local_variable
    MethodChannel _channel = const MethodChannel('multi_window_manager');
    log('MultiWindowManager initialized (secondary, reuse=$isEnabledReuseLocal)',
        name: 'MultiWindowManager');
    await _channel.invokeMethod('ensureInitialized', arguments);
    _current = MultiWindowManager._(windowId);
    _current?._isReusable = isEnabledReuseLocal;
    // Workaround for macOS: secondary window can stop rendering after the app
    // loses focus (Mission Control / cmd+tab). Flutter may disable frames on
    // AppLifecycleState.hidden; re-emitting inactive keeps frames enabled.
    if (Platform.isMacOS && windowId > 0) {
      WidgetsBinding.instance.addObserver(_SecondaryWindowRenderFix());
    }
  }
  ...
  Future<bool> isPreventClose() async {
    return await _invokeMethod<bool?>('isPreventClose') ?? false;
  }
  ...
  ValueNotifier<bool> isResizableNotif = ValueNotifier(true);
  ...
  Future<void> setResizable(bool isResizable) async {
    ...
    isResizableNotif.value = isResizable;
  }
  ...
  ValueNotifier<bool> isClosableNotif = ValueNotifier(true);
  ...
  Future<void> setClosable(bool isClosable) async {
    ...
    isClosableNotif.value = isClosable;
  }
  ...
  Future<void> startDragging() async {
    if ((Platform.isWindows || Platform.isLinux) && await isFullScreen())
      return;
...
class _SecondaryWindowRenderFix extends WidgetsBindingObserver {
  @override
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.hidden) {
      final sb = SchedulerBinding.instance;
      // ignore: invalid_use_of_protected_member
      sb.handleAppLifecycleStateChanged(AppLifecycleState.inactive);
    }
  }
}
