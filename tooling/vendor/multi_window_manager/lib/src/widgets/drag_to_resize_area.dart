...
class DragToResizeArea extends StatelessWidget {
    ...
    return Container(
      ...
      child: MouseRegion(
        ...
        child: GestureDetector(
          onPanStart: (_) =>
              MultiWindowManager.current.startResizing(resizeEdge),
          onDoubleTap: () => (Platform.isWindows &&
                  (resizeEdge == ResizeEdge.top ||
                      resizeEdge == ResizeEdge.bottom))
              ? MultiWindowManager.current.maximize(vertically: true)
              : null,
        ),
      ),
    );
  ...
  Widget build(BuildContext context) {
    return ValueListenableBuilder(
        valueListenable: MultiWindowManager.current.isResizableNotif,
        child: child,
        builder: (context, isResizable, child) {
          if (!isResizable) return child!;
          return Stack(
            children: <Widget>[
              child!,
              Positioned(
