...
//std::mutex threadMtx;
    ...
    private:
        ...
        void MultiWindowManagerPlugin::HandleScreenRetrieverMethodCall(
...
// static
    ...
    MultiWindowManagerPlugin::MultiWindowManagerPlugin(
        ...
        screen_retriever_channel_ =
                std::make_unique<flutter::MethodChannel<flutter::EncodableValue>>(
                        registrar->messenger(), "multi_window_manager/screen_retriever",
                                &flutter::StandardMethodCodec::GetInstance());
        screen_retriever_channel_->SetMethodCallHandler(
                [this](const auto& call, auto result) {
    ...
    std::optional<LRESULT> MultiWindowManagerPlugin::HandleWindowProc(
        ...
        if (wParam && message == WM_NCCALCSIZE) {
            ...
            // When the main window (id == 0) closes normally, shut the whole
            // process down: post WM_CLOSE to every secondary window so each
            // Flutter engine can clean up, then signal the message loop to exit.
            if (window_manager->id == 0) {
                for (auto& [wid, wm] : MultiWindowManager::windowManagers_) {
                    if (wid != 0 && wm->native_window != nullptr) {
                        PostMessage(wm->native_window, WM_CLOSE, 0, 0);
                    }
                }
                PostQuitMessage(0);
            }
    ...
    static BOOL CALLBACK mwm_enum_monitors_proc(HMONITOR hMonitor, HDC, LPRECT,
                                                 LPARAM lParam) {
        ...
        if (GetMonitorInfo(hMonitor, &mi)) {
            MwmMonitorData d;
            d.handle   = hMonitor;
            d.geometry = mi.rcMonitor;
            d.workarea = mi.rcWork;
    ...
    // Returns effective DPI for a monitor (MDT_EFFECTIVE_DPI = 0).
    // Falls back to 96 on pre-8.1 Windows where shcore is unavailable.
    static UINT mwm_get_dpi_for_monitor(HMONITOR monitor) {
        typedef HRESULT(WINAPI* PFN_GetDpiForMonitor)(HMONITOR, int, UINT*, UINT*);
        static PFN_GetDpiForMonitor fn = []() -> PFN_GetDpiForMonitor {
            HMODULE h = LoadLibraryW(L"shcore.dll");
            return h ? reinterpret_cast<PFN_GetDpiForMonitor>(
                           GetProcAddress(h, "GetDpiForMonitor"))
                     : nullptr;
        }();
        UINT dpiX = 96, dpiY = 96;
        if (fn) fn(monitor, 0 /*MDT_EFFECTIVE_DPI*/, &dpiX, &dpiY);
        return dpiX;
    }

    static std::string mwm_wchar_to_utf8(const wchar_t* wstr) {
        int len = WideCharToMultiByte(CP_UTF8, 0, wstr, -1, nullptr, 0,
                                      nullptr, nullptr);
        if (len <= 0) return {};
        std::string buf(static_cast<size_t>(len - 1), '\0');
        WideCharToMultiByte(CP_UTF8, 0, wstr, -1, buf.data(), len,
                            nullptr, nullptr);
        return buf;
    }

    static flutter::EncodableMap mwm_monitor_to_map(const MwmMonitorData& d) {
        constexpr double kBaseDpi = 96.0;
        double scale = mwm_get_dpi_for_monitor(d.handle) / kBaseDpi;

        // All values in logical (DPI-scaled) pixels, matching the original
        // screen_retriever formula exactly.
        double visX = std::round(d.workarea.left  / scale);
        double visY = std::round(d.workarea.top   / scale);
        double visW = std::round((d.workarea.right  - d.workarea.left) / scale);
        double visH = std::round((d.workarea.bottom - d.workarea.top)  / scale);
        // Width/height use rcMonitor.right/bottom minus visX/visY so that
        // secondary monitors at non-zero origins give correct dimensions.
        double w = std::round(d.geometry.right  / scale - visX);
        double h = std::round(d.geometry.bottom / scale - visY);
        ...
        if (GetMonitorInfo(d.handle, &miex)) {
            name = mwm_wchar_to_utf8(miex.szDevice);

            // Resolve the stable hardware DeviceID via EnumDisplayDevices,
            // same as the original screen_retriever_windows implementation.
            DISPLAY_DEVICE dd;
            dd.cb = sizeof(DISPLAY_DEVICE);
            int idx = 0;
            while (EnumDisplayDevices(miex.szDevice, idx, &dd, 0)) {
                if ((dd.StateFlags & DISPLAY_DEVICE_ACTIVE) &&
                    (dd.StateFlags & DISPLAY_DEVICE_ATTACHED_TO_DESKTOP)) {
                    std::wstring devName(dd.DeviceName);
                    if (devName.find(miex.szDevice) == 0) {
                        id = mwm_wchar_to_utf8(dd.DeviceID);
                        break;
                    }
                }
                ++idx;
        ...
        flutter::EncodableMap size = {
            {flutter::EncodableValue("width"),  flutter::EncodableValue(w)},
            {flutter::EncodableValue("height"), flutter::EncodableValue(h)},
        };
        flutter::EncodableMap visPos = {
            {flutter::EncodableValue("dx"), flutter::EncodableValue(visX)},
            {flutter::EncodableValue("dy"), flutter::EncodableValue(visY)},
        };
        flutter::EncodableMap visSize = {
            {flutter::EncodableValue("width"),  flutter::EncodableValue(visW)},
            {flutter::EncodableValue("height"), flutter::EncodableValue(visH)},
        };

        return {
            {flutter::EncodableValue("id"),              flutter::EncodableValue(id)},
            {flutter::EncodableValue("name"),            flutter::EncodableValue(name)},
            {flutter::EncodableValue("size"),            flutter::EncodableValue(size)},
            {flutter::EncodableValue("visiblePosition"), flutter::EncodableValue(visPos)},
            {flutter::EncodableValue("visibleSize"),     flutter::EncodableValue(visSize)},
    ...
    void MultiWindowManagerPlugin::HandleScreenRetrieverMethodCall(
        ...
        // Dart always passes devicePixelRatio for every call.
        double device_pixel_ratio = 1.0;
        if (method_call.arguments() && !method_call.arguments()->IsNull()) {
            const auto* args =
                std::get_if<flutter::EncodableMap>(method_call.arguments());
            if (args) {
                auto it = args->find(flutter::EncodableValue("devicePixelRatio"));
                if (it != args->end()) {
        ...
        if (method == "getCursorScreenPoint") {
            ...
            flutter::EncodableMap map = {
                {flutter::EncodableValue("dx"),
                 flutter::EncodableValue(pt.x / device_pixel_ratio)},
                {flutter::EncodableValue("dy"),
                 flutter::EncodableValue(pt.y / device_pixel_ratio)},
            };
            ...
            // Build a MwmMonitorData for the primary monitor so we can reuse
            // mwm_monitor_to_map.
            MONITORINFO mi;
            mi.cbSize = sizeof(MONITORINFO);
            if (GetMonitorInfo(primary, &mi)) {
                ...
                d.workarea = mi.rcWork;
                d.index    = 0;
                result->Success(flutter::EncodableValue(mwm_monitor_to_map(d)));
            } else {
            ...
            EnumDisplayMonitors(nullptr, nullptr, mwm_enum_monitors_proc,
                                reinterpret_cast<LPARAM>(&monitors));
            flutter::EncodableList list;
            for (const auto& d : monitors) {
                list.push_back(flutter::EncodableValue(mwm_monitor_to_map(d)));
            }
