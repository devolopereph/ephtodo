...
![Demo](https://raw.githubusercontent.com/vvlad-islavs/multi-window-manager/master/example.gif)
...
1. Add the plugin header include (alongside the other includes at the top of the file):
...
2. In `my_application_activate`, replace `gtk_widget_realize(GTK_WIDGET(window))` (which appears before the view is created) with a realize call on the view after it is added to the window, and add the two MWM calls around `fl_register_plugins`:
...
+  multi_window_manager_linux_init(GTK_APPLICATION(application), fl_register_plugins);
...
`multi_window_manager_linux_init` holds the `GtkApplication` alive (so closing a secondary window does not quit the process) and registers the internal callback that creates new GTK windows when Dart calls `MultiWindowManager.createWindow`.

`multi_window_manager_linux_detach_flutter_quit_on_window_close` removes Flutter's built-in delete-event handler, which would otherwise call `g_application_quit` whenever any window is closed.

The realize order matters: realizing the window before adding the `FlView` causes GTK to allocate it at the default 1280x720 size immediately, so Flutter starts rendering at that size before Dart has a chance to call `setSize` or `setTitleBarStyle`. Realizing through the view after `gtk_container_add` defers GDK window creation until the full widget hierarchy is in place.

> **Note:** if the generated `my_application.cc` in your project does not have `gtk_widget_realize(GTK_WIDGET(window))`, skip the line removal and the realize change - just add the three `+` lines.

#### Linux limitations

- **reuseMode is forced on Linux.** Due to Flutter engine lifecycle constraints, destroying and re-creating a `FlView` / `FlEngine` pair while the `GtkApplication` is still running is not safe and causes crashes. The library automatically forces reuse mode for all secondary windows on Linux regardless of the `isEnabledReuse` flag. Because of this, prefer `createWindowOrReuse` over `createWindow` on Linux - it is both faster (no engine allocation) and avoids creating windows that cannot be properly destroyed.

- Position and alignment APIs (`setPosition`, `setAlignment`, `center`, `dock`) require **X11**. They return silently on Wayland.

- `setAlwaysOnTop` and `setAlwaysOnBottom` require **X11**.

- `setMovable`, `setProgressBar`, `setBadgeLabel`, `setVisibleOnAllWorkspaces`, `hasShadow`/`setHasShadow`, and `setIgnoreMouseEvents` are not supported on Linux.
...
> **Linux:** reuse mode is forced for all secondary windows regardless of `isEnabledReuse`. Prefer `createWindowOrReuse` over `createWindow` - it is faster and avoids engine allocation on every call.
...
No extra setup required. All windows (primary and secondary) are created hidden by the library and stay hidden until Dart calls `show()` inside `waitUntilReadyToShow`.
